...
if (body?.summarize_data?.repository_data) {
  ...
  // Handle manual data triggering with payload.data structure
  if (payload?.data && payload.data.commit) {
    const commit = payload.data.commit;
    ...
    const modifiedFiles = payload.data.files ? payload.data.files.map(file => file.filename) : [];
    htmlContent = `
      <p><strong>Author Name:</strong> ${author.name}</p>
      <p><strong>Email ID:</strong> ${author.email}</p>
      <p><strong>Commit ID:</strong> ${payload.data.sha ?? "N/A"}</p>
      ...
      <p><strong>AI Summary:</strong> ${formatSummaryToHtml(aiSummary, "No AI summary available.")}</p></br></br></br>
      <p><strong><i></i>This is automatically generated by <u>BAS Services.</u></strong></p>
    `;
